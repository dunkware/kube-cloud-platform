...
EUREKA_CHART_NAME='mvp-test-auth'

kubectl create namespace ${NAMESPACE} || true
kubectl delete configmap ${EUREKA_CHART_NAME} -n ${NAMESPACE} || true
kubectl create configmap ${EUREKA_CHART_NAME} --namespace=${NAMESPACE} --from-file=$(pwd)/config/auth || true
