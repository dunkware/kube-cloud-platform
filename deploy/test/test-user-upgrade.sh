...
EUREKA_CHART_NAME='mvp-test-user'

kubectl create namespace ${NAMESPACE} || true
kubectl delete configmap ${EUREKA_CHART_NAME} -n ${NAMESPACE} || true
kubectl create configmap ${EUREKA_CHART_NAME} --namespace=${NAMESPACE} --from-file=$(pwd)/config/user || true
